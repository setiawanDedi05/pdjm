...
import { Minus, Plus, Trash2, ShoppingCart, Printer, Banknote, CreditCard, BanknoteXIcon, PackagePlusIcon, PlusCircleIcon, MinusCircleIcon } from 'lucide-react';
...
export default function KasirPage() {
  ...
  async function submitCheckout(status?: string, midtransOrderId?: string) {
    const payload = {
      ...
      total_price: totalPrice() + serviceFee.reduce((sum, fee) => sum + fee.servicePrice, 0),
  ...
  async function handleNonCashPayment() {
    ...
    const tokenRes = await fetch('/api/midtrans/token', {
      ...
      body: JSON.stringify({
        order_id: orderId,
        gross_amount: totalPrice() + serviceFee.reduce((sum, fee) => sum + fee.servicePrice, 0),
        customer_name: customerName,
        vehicle_plate: vehiclePlate,
        payment_method: paymentMethod
      }),
  ...
  const handleServicePriceChange = (index: number, value: string) => {
    ...
    const finalValue = cleanValue === '' ? 0 : Number(cleanValue);
    const updatedFees = [...serviceFee];
    // @ts-ignore
    updatedFees[index].servicePrice = finalValue;
    setServiceFee(updatedFees);
  };

  const handleServiceNameChange = (index: number, value: string) => {
    const updatedFees = [...serviceFee];
    // @ts-ignore
    updatedFees[index].serviceName = value.trim();
    setServiceFee(updatedFees);
  };

  const handleRemoveServiceFee = (index: number) => {
    const updatedFees = [...serviceFee];
        ...
        <Card className="flex-1">
          ...
          <CardContent>
            ...
            <div className='flex items-center gap-2 mb-2'>
              ...
              <button onClick={() => setServiceFee([...serviceFee, { serviceName: '', servicePrice: 0 }])}
            ...
            <div className="space-y-2 mb-2">
              {serviceFee.map((fee, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  <Input
                    type="text"
                    placeholder="Biaya Jasa"
                    value={Number(fee.servicePrice)}
                    onChange={(e) => handleServicePriceChange(index, e.target.value)}
                  />
                  <button onClick={() => handleRemoveServiceFee(index)}
                    className="p-1 rounded-full hover:bg-slate-200"
                  >
                    <MinusCircleIcon className="h-5 w-5 text-slate-500" />
                  </button>
                </div>
              ))}
              {serviceFee.length > 0 && (
              ...
              {serviceFee.length > 0 && (
                <div className="flex justify-between text-xs text-slate-600">
                  ...
                  <span>{formatCurrency(serviceFee.reduce((sum, fee) => sum + fee.servicePrice, 0))}</span>
                </div>
              )}
            </div>
            <Separator className="my-2" />
